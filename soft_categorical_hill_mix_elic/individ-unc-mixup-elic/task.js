...
var condition_num = 3; // just use one for now to make sampling easier later
...
var instructions = {
	...
	'<p> This experiment should take at most <strong>25 minutes</strong>. </br></br> You will be compensated at a base rate of $8/hour for a total of <strong>$3.34</strong>, which you will receive as long as you complete the study.</p>',
			...
			'<p> In this experiment, you will be seeing low-resolution <i>images</i>.</p>'+
      '<p> These images are the result of <i>combining</i> images of various object classes (e.g., a mixture of a Truck and a Cat).</p>' +
      '<p> Your task will be to imagine that others are looking at these images and asked to figure out what category the image most probably represents. You can assume they don\'t know the images are a combination of classes.</p>'+
      '<p> What categories do you think they would select are the most probable categories represented in the image? And are there any categories you think they would definitely say the image is <i>not</i>?</p>' +
      '<p> Specifically, please image <strong>100 crowdsourced workers</strong> are doing this task. Consider how they may respond to each image.</p> ',
			// '<p> Each image depicts an instance of one or more images from a given <i>category</i>, e.g., a dog, a truck, an airplane.</p>' +
      // '<p> You will have <strong>multiple tasks per image</strong>, including coming up with the most probable, second most probable, and improbable categories for the image. Along with associated estimates of the probabilities of those categories being the correct category of the image. </p>',
			...
			'<p> Your first task then will be to <strong>select the most probable category</strong> you think the other crowdsourced workers would believe is represented in the image.</p>' +
      ...
      '<p> For instance, you may think that an image would be categorized as an automobile, or maybe a truck but definitely not as a dog, cat, or frog. In that case, please click dog, cat, <i>and</i> frog.</p>', //<p>Please skip this question only if you think that the 100 crowdsourced workers would think that all categories could possibly be the true category shown in the image.</p>',
			...
			'<p> The categories of some images may be obvious, and you may think that other crowdsourced workers would definitely perceive a particular category in the image. </p>' +
...
var main_page = {
    ...
    html: function() {
      ...
      + '<label for='+noneOption+'>' + noneOption + '</label></div>'
			...
			var probEnter = '<p><center>If you selected an alternate category for the image, what <strong>percent probability (between 0 and 100)</strong> do you think they would assign to the category you selected being the true category represnted in the image?' // </center></p>'
